...
fprintf('RT St. Deviation: %.0f +- %.0f ms\n', mean(perf_all.IDED.RT.std_all(is_young)) * 1000, std(perf_all.IDED.RT.std_all(is_young)) * 1000)
...
fprintf('RT St. Deviation: %.0f +- %.0f ms\n', mean(perf_all.IDED.RT.std_all(is_old)) * 1000, std(perf_all.IDED.RT.std_all(is_old)) * 1000)
...
% use second repeat as control condition
% separate by condition
dv = [perf_all.IDED.RT.std_repeat2(is_young,:); perf_all.IDED.RT.std_repeat2(is_old,:); ...
    perf_all.IDED.RT.std_ID(is_young,:); perf_all.IDED.RT.std_ID(is_old,:); ...
    perf_all.IDED.RT.std_ED(is_young,:); perf_all.IDED.RT.std_ED(is_old,:)];
subj_num = [1:numel(subs) 1:numel(subs) 1:numel(subs)]';
...
T.gender =  [gender(is_young)' - 1; gender(is_old)' - 1; gender(is_young)' - 1; gender(is_old)' - 1; ...
    gender(is_young)' - 1; gender(is_old)' - 1;];
writetable(T, strcat(dirs.output_dir, '4_Stats\stat_RTstd_repeat_ID_ED.csv'));
