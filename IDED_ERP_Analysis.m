...
%
% extract epochs, response locked
% cfg.trialdef.prestim      = 2.5; % in seconds
% cfg.trialdef.poststim     = 0.5; % in seconds
% cfg.trialdef.eventvalue   = {'S 31', 'S 32'};
% cfg.trialdef.trigger_type = 'previoustrigger';
% cfg_response              = ft_definetrial(cfg);
...
save(strcat(dirs.prepr_dir, subj, '_IDED_ERP_ft_prepr_1.mat'), "subjectdata");
fprintf('succesful!\n\n')
end
...
%%
%----------------------------------------------------------------------
%           Calculate subject averages for each condition
%----------------------------------------------------------------------
...
%%
%----------------------------------------------------------------------
%           Generate struct array to save all subject averages
%----------------------------------------------------------------------
...
save(strcat(dirs.output_dir, '2_Subj_Avg\all_subj_avg_stimpres_ERP'),...
    ...
    "all_ED_stimpres_ERP", "all_alltrl_stimpres_ERP");
...
% plot erps
figure()
for i = 1:sum(is_young)
    hold on
    plot(young_alltrl_stimpres_ERP{i}.time, young_alltrl_stimpres_ERP{i}.avg(1,:))
end

figure()
for i = 1:sum(~is_young)
    hold on
    plot(old_alltrl_stimpres_ERP{i}.time, old_alltrl_stimpres_ERP{i}.avg(1,:))
end
...
save(strcat(dirs.output_dir, '2_Subj_Avg\young_subj_avg_stimpres_ERP'),"young_repeat1_stimpres_ERP", ...
    ...
    "young_ED_stimpres_ERP", "young_alltrl_stimpres_ERP");
...
save(strcat(dirs.output_dir, '2_Subj_Avg\old_subj_avg_stimpres_ERP'), "old_repeat1_stimpres_ERP", ...
    ...
    "old_ED_stimpres_ERP", "old_alltrl_stimpres_ERP");
...
%%
%----------------------------------------------------------------------
%          Calculate grand average ERP for each condition
%----------------------------------------------------------------------
...
save(strcat(dirs.output_dir, '3_Grandavg\grandavg_ERP_stimpres.mat'), "ERP");

%%
%----------------------------------------------------------------------
%          Collect average amplitudes for specific EEG components
%----------------------------------------------------------------------
...
% clear unnecesarry variables
clear P300p_avg max_P300p P300f_avg max_P300f N200p_avg min_N200p
