...
freq_avg = ft_checkdata(freq_avg, 'datatype', {'freq+comp', 'freq'}, 'feedback', 'yes');
...
if size(cfg.baseline,1)==numel(freq_avg.freq) && size(cfg.baseline,2)==2
  % this is ok
...
for k = 1:numel(cfg.parameter)
  ...
  if strcmp(freq.dimord, 'chan_freq_time')
    ...
    freqOut.(par) = ...
      performNormalization(freq.time, freq.(par), freq_avg.(par), cfg.baseline, cfg.baselinetype);
  ...
  elseif strcmp(freq.dimord, 'rpt_chan_freq_time') || strcmp(freq.dimord, 'chan_chan_freq_time') || strcmp(freq.dimord, 'subj_chan_freq_time')
    ...
    for l = 1:size(freq.(par), 1)
      ...
      freqOut.(par)(l,:,:,:) = ...
        performNormalization(freq.time, tfdata, freq_avg.(par), cfg.baseline, cfg.baselinetype);
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function data = performNormalization(timeVec, data, data_avg, baseline, baselinetype)
...
% compute mean of time/frequency quantity in the baseline interval from averaged data,
% ignoring NaNs, and replicate this over time dimension
if size(baselineTimes,1)==size(data_avg,2)
  % do frequency specific baseline
  meanVals = nan+zeros(size(data_avg));
  for k = 1:size(baselineTimes,1)
    meanVals(:,k,:) = repmat(nanmean(data_avg(:,k,baselineTimes(k,:)), 3), [1 1 size(data_avg, 3)]);
  end
else
  meanVals = repmat(nanmean(data_avg(:,:,baselineTimes), 3), [1 1 size(data_avg, 3)]);
end
